...
- **Two philosophical anchors for all sync work:** (1) `6f4f5b0` — the May 13 2026 commit, the simplest engine that ever worked: no playbackRate touching, just `if (drift > 0.3) seekPreservingBT(expected)` inside a 10s interval. When lost, return here. (2) `baseline-sync-jun16` — the current production ceiling: same spirit plus the `playing` re-anchor for stall recovery. Every improvement attempt must beat both on real CSV data (median drift, stallCount, snapCount). More complexity is never the answer — this engine got to 1ms by removing code, not adding it
