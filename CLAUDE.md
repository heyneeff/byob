...
| `ENTRY_AND_ORCHESTRA.md` | The entry→scenes→orchestra plan (written 2026-07-16 from the Jul 15 evidence day): tight entry campaign (M2), Ableton scene launch (arm-then-fire), orchestra track differentiation. **Read before any entry/launch/scene work** — it carries the step sequence and which steps require casts |
...
- **`baseline-sync-jul6` tag (commit `1016005`) — Jul 6 2026. NEW BENCHMARK.** Anchor broadcast: DJ heartbeats live position every 5s on sync channel, all listeners derive `playbackStartedAt` from it — eliminates per-device clock-offset divergence. Launch guard: `fastDriftCorrect` suppressed until anchor arrives after track load (clears early on anchor receipt, but only when `_trackLoading=false`). Auto-resync: DJ self-checks drift every 5s and fires `triggerResync` if >500ms off reference. Live result: inter-device spread collapsed from ±75-124ms (4 devices in 4 different zones) to ±36-73ms converging toward a single reference. Clean synced track entry confirmed on hardest test track.
- **`baseline-sync-jun16b` tag (commit `bf76374`) — prior validated floor** — SEEK_STAB_S=0.19, triggerResync uses existing reference. Any regression must be caught by diffing against `baseline-sync-jul6` first, then `baseline-sync-jun16b`.
- **`baseline-sync-jul14` tag (commit `a1d80a4`) — Jul 14 2026.** The honest-signal room: `computeOwnRefMs` devLat sign fixed (`daab95f` — converged phones had been reporting `trackStart + 2·devLat`, so cascade consensus orbited on latency echoes; see TUNING_LOG "cascade signal/lever mismatch"). Live: zero false cascade fires, one-shot rescues that settle, natural flagship emergence, first "sync is perfect" report. Regression guard: `sync/octonary-cascade-sim.mjs` — H1b must keep reproducing the pre-fix orbit, H1/H2/H3 must stay quiet.
- **`baseline-sync-jul15` tag (commit `d32faec`) — Jul 15 2026. CURRENT BENCHMARK: THE STABLE ROOM.** Marked by the user live ("the most stable it's ever been") on `byob-obs-2026-07-15T12-46-54-201Z.csv`: 4 settled phones at |drift| p50 54–85ms / p95 71–122ms, room refMs spread p50 33ms pinned flat 33–34ms for the final 90s, three deliberate SYNCED ground-truth markers, warp duty ~0% on settled phones, all devices cap-pinned lat=1200 (the accidental-equalizer regime — do not "fix" naively). Also the guileless-clock generation's calm-stretch A/B: PASS (p95 tail 181–349 → 71–122ms). **Known weakness, next target: rough entry** — launch propagation staggered 34s across devices at 12:39; both late joiners never settled. Pre-warp-gate code (the gate `984c34c` deployed after this room's last refresh).
- **Six sync reference points — always return here when lost:**
  ...
  - `bf76374` (`baseline-sync-jun16b`) — validated ceiling: SEEK_STAB_S=0.19, triggerResync uses existing reference. Every improvement must beat all four on real CSV data. More complexity is never the answer.
  - `1016005` (`baseline-sync-jul6`) — anchor broadcast era: DJ live position as ground truth, launch guard, auto-resync.
  - `a1d80a4` (`baseline-sync-jul14`) — honest-signal era: refMs reconstruction sign fixed, cascade corrects only true divergence.
  - `d32faec` (`baseline-sync-jul15`) — the stable room: p95 tails collapsed, refSpread pinned at 33ms, user-marked. Entry still rough. Current production ceiling.
...
- **The well gauge observes; it never acts.** `deviceHexagram()` (ternary/layer.js) and the `hexKw`/`hexLines`/`hexMoving` broadcast fields are observe-only by design law (casts 48 / 42.1.4→12 / 51.2.3→34, 2026-07-14 — the resolved hexagram of the implementation cast is Standstill). No correction, gate, weight, threshold, or any engine read-path may ever consult them. Flow is upward only: phone → broadcast → overlay/console. If you are considering feeding the gauge back into behavior: don't — that is the corruption 12 warns about.
