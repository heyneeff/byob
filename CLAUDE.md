...
- **`baseline-sync-jun14` tag (commit `481988b`)** is the current validated-stable sync engine — supersedes `baseline-sync-jun13` (commit `e6bb58e`). Live-tested Jun 14 2026: Phase 5r micro-correction holds tight with no roving, plus the Deck A play-state and zone-slot preview fixes. Any change to `sync/sync-engine.js` or `fastDriftCorrect`/`requestCorrection`/the seek formula in `listener.html` must be diffed against this tag, validated in `sync-sim.html`, and live-tested before merging to `main`. If a change regresses drift/warping, `git revert` it rather than layering a fix on top — see Jun 13 2026 `b683c24` background-catchup incident in the Roadmap session log for why
