...
- **`baseline-sync-jun17` tag (commit `d9cf8e1`) is the current state** — supersedes `baseline-sync-jun16b`. Built Jun 17 2026. Does NOT change the sync engine core. Adds: (1) drift-based latency self-calibration (`_selfCalibrateTick` in listener.html — silently walks `_deviceLatencyMs` toward zero mean drift over ~100s stable windows, no mic needed); (2) outputLatency seed for uncalibrated Chrome/Android devices; (3) `_djSyncChannel` unsubscribe-before-reassign fix eliminating double hard_sync delivery; (4) `_maybeSpatialResync` — auto-fires `triggerResync()` 4s after spatial launch with new stems; (5) live drift chart in debug.html. Self-cal is additive to the sync engine — it only adjusts `_deviceLatencyMs` in localStorage and resets on snaps, never touches the corrector or seek formula. Needs a live validated CSV before promoting to confirmed baseline.
